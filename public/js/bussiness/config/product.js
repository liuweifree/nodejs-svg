        ...
        if(data.result == 200) {
            ...
            for (var i = 0; i < 5; i++) {
                ...
                if( paramSize > i){
                    ...
                    discount = data.data.paramList[i].discount;
                }
                htmlStr += '<div class="control-group" style="margin-top:18px;">\n' +
                    '                                                <div style="margin-left: 20px;">\n' +
                    '                                                    <div>\n' +
                    '                                                        产品ID:<input type="text" class="span3 adId" value="'+pid+'" >\n' +
                    '                                                        折扣:<input type="text" class="span3 adPosition"  value="'+discount+'">\n' +
                    '                                                    </div>\n' +
                    '                                                </div>' +
                    '                                            </div>';
...
var updateAd = function(){
    ...
    for(var i=0;i<adIdList.length;i++){
        if($(adIdList[i]).val() && '' != $(adIdList[i]).val()){
            ...
            param.discount = $(adPosition[i]).val();
